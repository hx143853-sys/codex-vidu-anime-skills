...
- full episode/chapter script or the largest available script section,
- storyboard table,
- all storyboard rows in the same episode/chapter or scene, not only the target row,
...
Do not rely only on one storyboard row or the storyboard `出场角色` column. A single row may omit visible targets, off-screen geography, scene direction, props, monsters, UI, vehicles, weapons, or effects that are clear from the full script and adjacent shots. Before writing final prompts, understand the episode/scene context and use it to repair reasonable omissions without changing the plot.

If the user only provides isolated rows and asks for final production prompts, say the output is a context-risk draft unless the missing script/episode context is supplied. Do not invent continuity that cannot be inferred.
...
## Episode Context Pass

Before the `主体调用计划` and final prompts, perform an episode/scene context pass from the full script, all available storyboard rows, and neighboring shots.

For production prompts, maintain or output a compact `本集上下文理解` containing:

- `剧情进展`: what has happened before the target range and what the target range is trying to show,
- `场景空间地图`: key locations and how they relate, such as upstairs/downstairs, inside/outside, window/door/street/store entrance,
- `角色状态`: where important characters are, what they know, what they are holding, injuries/fear/intent, and who they are watching,
- `威胁/目标位置`: where zombies, enemies, vehicles, UI, props, or targets are located,
- `镜头连续性`: what visible state carries across shots, such as same room, same window, same line of sight, same blocked doorway, same crowd outside,
- `分镜补全依据`: details inferred from script or adjacent shots that are missing from a sparse storyboard row.

Use this context to write each shot. Example: if a storyboard row only says `场景=卧室，出场角色=小明，画面描述=小明观察窗外`, but the episode script and neighboring shots establish that 小明 is crouching beside an upstairs bedroom window and watching zombies at the downstairs convenience-store entrance, the prompt must describe that line of sight and target. Do not turn the shot into 小明 looking at zombies inside the bedroom.

If the context conflicts with the storyboard, do not silently choose the more dramatic version. Mark the conflict in `备注` or ask for confirmation before final prompts.
...
首帧：<exact opening frame: shot size, camera viewpoint, subject positions, line of sight, foreground/midground/background, visible target, lighting/effects>.
0-X秒：<concrete visible first beat with exact framing, subject entry or held position, gaze, action, camera movement, environment/effect motion, and beat ending state>.
X-Y秒：<concrete visible second beat with motion change, camera path, subject action/reaction, target movement, entry/exit/hold, and beat ending state>.
<final quality words>.
```
...
首帧：超远景，镜头贴近废墟街道地面，[@银黑色巨型未来机甲]站在画面左侧前景，[@黑色红光巨型怪兽]压在右侧楼体残骸后方，碎裂街道形成中间空线，烟尘从两侧楼体间缓慢飘过。
0-2秒：低机位广角镜头沿街道中线缓慢推进，[@银黑色巨型未来机甲]右臂微微抬起进入画面中心，[@黑色红光巨型怪兽]从残骸阴影后探出上半身，结束时双方隔着街道形成正面对峙。
2-4秒：镜头硬切到驾驶舱特写，[@白发少女驾驶员]坐在驾驶座中央，双手握紧操纵杆，抬眼看向前方红色警报光和远处怪兽投影，驾驶舱红光扫过脸部，镜头轻微震动，结束时[@白发少女驾驶员]的眼神收紧。
...
3. Add `首帧：` before time beats. This opening frame must be a complete still image that can generate the first frame: camera viewpoint, shot size, subject positions, visible target, line of sight, frame depth, lighting, and important objects.
4. Use time-coded beats as separate lines: `0-1秒：`, `1-3秒：`, `3-5秒：`, etc.
5. Every time beat must describe a concrete visible image, subject entry/hold/exit, camera move, action change, and ending state. Do not write abstract placeholders like `建立起始构图`, `推动主要动作`, `停在结果反应`, `画面准备切入`, or `当前信息压力`.
6. Include exact framing and camera language inside the beat itself: `低机位近景`, `中景缓慢后拉`, `浅景深特写推近`, `快速横移`, `门缝视角`.
7. Include subject position and gaze target inside the beat: left/right/front/back/foreground/background only when useful, and exact targets like `[@角色B]看向[@角色C（感染后）]`.
8. If the storyboard uses sound-first or black-screen narration, convert it into a visible silent-video equivalent. Do not rely on hearing sound. Use visual suspense such as dark frame fade-in, trembling hand, mouth opening, door crack, shadow movement, subtitle-free panic expression, or a silent scream.
9. Do not include full dialogue unless the user needs lip-sync. If dialogue exists, describe visible mouth movement and speaker only: `[@角色B]嘴唇颤抖地说话，非说话角色不张口`.
10. Put negative constraints in the `负面约束` column, not inside the final prompt, unless visually necessary.
...
- simple 5-second shot: about 220-380 Chinese characters when references are stable,
- complex observation/dialogue/action shot: about 380-650 Chinese characters,
- 12-15 second grid/sequence shot: about 650-1000 Chinese characters.

If too long, remove repeated style words and still-image clothing details that are already anchored by references. Keep the episode-context repair, `首帧`, spatial relationship, camera movement, subject entry/exit, ending state, subject references, and continuity.
...
- `上下文补全`: what full script / episode storyboard clarifies beyond the current row.
...
- `首帧设计`: the first visible still frame before motion starts.
...
- `入镜/出镜`: who or what enters, exits, stays hidden, or remains still during the shot.
...
Sparse storyboard rows require context repair. If the script or adjacent shots establish a concrete target, location, or line of sight, put it into the prompt even when the row is brief. If the script does not establish it, mark the row as ambiguous instead of guessing.
...
## Cinematic And Anime Direction Library

Use these as a selection library, not as a tag pile. Pick the few techniques that fit the story beat, and always attach them to visible subjects and camera action.

Shot size and angle:

- `超远景 establishing shot`, `远景`, `全景`, `中景`, `近景`, `特写`, `大特写`, `过肩镜头`, `背影镜头`, `门缝视角`, `窗边侧脸特写`, `低机位仰拍`, `高机位俯拍`, `主观视角`, `长焦压缩空间`, `广角压迫构图`.

Camera movement:

- `缓慢推近`, `快速推近`, `后拉揭示`, `横移跟拍`, `环绕半圈`, `上摇到眼神`, `下移到手部`, `焦点从前景转到后景`, `手持轻微晃动`, `冲击瞬间震动`, `快速切入`, `硬切特写`, `慢动作定格`.

Anime and film effects:

- `速度线`, `动作残影`, `冲击波扩散`, `灰尘颗粒漂浮`, `碎片飞散`, `玻璃反光`, `窗框阴影切割脸部`, `冷光扫过眼睛`, `红色警报光闪烁`, `浅景深背景虚化`, `前景遮挡制造窥视感`, `逆光轮廓`, `高反差阴影`, `焦点呼吸`, `空气热浪/雾气扩散`.

Suspense and observation:

- `门框压住画面边缘`, `窗帘遮住半张脸`, `镜头贴近肩后看向远处目标`, `前景黑影遮挡三分之一画面`, `目标在远景入口缓慢移动`, `主体屏住呼吸保持不动`, `手指收紧但身体不动`.

Do not use a technique as decoration. For example, do not write only `动态镜头、电影感、压迫感`; write what the camera does and what visible subject or target it emphasizes.
...
- the full episode/script/storyboard context has been reviewed, or the output is clearly marked as a context-risk draft,
- production route is clear or reasonably inferred,
- prompt is dynamic, not just a still-image description,
- prompt includes a complete `首帧：` line before the time beats,
- prompt has a beginning, middle, and ending state,
- prompt uses separate time-coded segments such as `0-2秒：` and `2-5秒：`,
- every time-coded segment includes visible image, subject entry/hold/exit, camera movement, action change, and ending state,
