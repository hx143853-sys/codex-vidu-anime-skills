...
- `zombie-video-prompting`: 分镜转视频提示词，先输出主体调用计划并确认，再生成可直接用于视频模型的分段时间轴动态提示词，支持 Vidu 主体引用 `[@主体名]`。
- `zombie-vidu-generation`: Vidu 生成执行层，负责提交前检查、参数映射、真实 prompt 日志、任务日志、查询、下载和失败处理。
...
8. 用户确认提示词和提交范围后，再调用 `zombie-vidu-generation` 提交 Vidu 任务。
9. 成片后调用 `zombie-shot-review-redo` 做返工表和重做提示词。
