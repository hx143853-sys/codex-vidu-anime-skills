...
- `zombie-storyboard-director`: 剧本转分镜表，逐镜头判断真实出场角色，并生成镜头号、时长、景别、场景、出场角色、导演级画面描述、台词、音效、备注。
- `zombie-frame-grid-planning`: 首帧图和宫格图规划，用于首帧图生视频、单图转动态视频、宫格图生视频路线；参考主体必须在提示词正文中内联引用。
- `zombie-video-prompting`: 分镜转视频提示词，先输出主体调用计划并确认，再生成可直接用于视频模型的分段时间轴动态提示词，支持按 Vidu 当前工具要求在正文中内联引用真实主体。
- `zombie-vidu-generation`: Vidu 生成执行层，负责账号主体检查、缺失主体创建、提交前检查、参数映射、真实 prompt 日志、任务日志、查询、下载和失败处理。
...
6. 调用 `zombie-video-prompting` 先生成“主体调用计划”，确认每个镜头的确认调用主体、疑似应调用主体、排除主体和判断原因。
7. 主体调用计划确认后，再生成分段时间轴视频提示词。
8. 用户确认提示词和提交范围后，调用 `zombie-vidu-generation` 先检查绑定 Vidu 账号是否已有对应主体；缺失主体需要用户提供 1-3 张图片并确认创建。
9. 主体齐全后再提交 Vidu 任务，默认编码使用用户可见 `H264_PRO`，CLI/API 参数为 `h264_pro`。
10. 成片后调用 `zombie-shot-review-redo` 做返工表和重做提示词。

## 关键规则

- 每一步生成、上传、提交 Vidu、下载替换文件前，都需要用户确认。
- 不要只看分镜的 `出场角色` 列，要结合剧本上下文、画面描述、视线关系、动作目标和素材表判断真实出场主体。
- 分镜 `画面描述` 必须是可拍的自然语言：写清谁在哪里、做什么、什么表情姿态、看向谁或什么、镜头如何拍；不要用 `其`、`听者`、`情绪重量`、`强化张力`、`接上一镜头` 这类抽象或代指话术。
- 分镜 `画面描述` 要把场景标题转成自然地点，例如 `别墅客厅中`，不要把 `1-2 日 内` 这类场次编号写进画面。
- 视频提示词生成前必须先产出“主体调用计划”，确认后才进入最终提示词和 Vidu 提交。
- Vidu 已保存主体必须在提示词正文中出现的位置使用当前 Vidu 工具要求的真实主体引用格式，并在素材映射中有对应元素；不要把 `@主体名`、`[@name]`、`[@角色A]` 这类占位符当成真实输出，也不要把主体列表追加在提示词末尾代替正文引用。
- 不要写 `[@角色] 是角色参考图` 这种身份解释句，主体 token 本身就是身份锚点。
- 最终视频提示词应使用分段时间轴动态提示词：先写风格、场景和氛围句，再写 `0-2秒：...`、`2-5秒：...` 等动作段落；每段都要写清画面、主体、运镜、动作变化和结束状态。
- 提示词必须写清空间关系：镜头在哪里、主体在哪里、主体看向/接近/躲避什么、目标在前景/中景/远景或室内/室外/楼上/楼下的哪一侧。
- 最终视频提示词不能包含未解决的决策词，例如“或者、可选择、如果需要、生成/选择、建议使用、看情况”等；这些只能作为 agent 思考或备注。
- Vidu 提交计划、日志和总结必须保留真实 prompt 文本或摘要；任务日志应区分 `prompt_text`、`prompt_file`、`cli_prompt_arg`。
- Vidu 提交前必须检查绑定账号主体；缺失主体先请求图片并创建 element，不要静默改成普通文本或把主体引用删掉。
