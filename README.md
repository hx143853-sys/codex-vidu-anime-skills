...
6. 调用 `zombie-video-prompting` 先阅读本集剧本/本集分镜/相邻镜头，形成上下文理解，再生成“主体调用计划”。
7. 主体调用计划确认后，再生成带 `首帧：`、空间关系、入镜/出镜、景别、运镜和动漫/影视效果的分段时间轴视频提示词。
...
- 视频提示词不能只看单个分镜行；必须结合本集剧本、完整分镜、剧情发展和相邻镜头补全合理空间与目标，不确定时标记为上下文风险或询问。
...
- 最终视频提示词必须包含完整 `首帧：`，并写清主体如何保持、入镜、出镜、移动或被镜头揭示。
